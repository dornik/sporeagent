...
for i = 1:numel(keyframes)
    ...
    % load PoseCNN result
    filename = sprintf('results_sporeagent-ilrl/%06d.mat', i - 1);
    ...
    for j = 1:numel(gt.cls_indexes)
        ...
        if isempty(roi_index) == 0
            ...
            % pose
            RT(1:3, 1:3) = quat2rotm(result.poses_ours(roi_index, 1:4));
            RT(:, 4) = result.poses_ours(roi_index, 5:7);
            ...
            if sum([13, 16, 19, 20, 21] == cls_index) == 0
                distances_ad(count, 1) = distances_non(count, 1);
            else
                distances_ad(count, 1) = distances_sys(count, 1);
            end            
        else
            distances_sys(count, 1:4) = inf;
            distances_non(count, 1:4) = inf;
            distances_ad(count, 1:4) = inf;
...
distances_ad = distances_ad(1:count, :);
...
save('results_sporeagent-ilrl.mat', 'distances_sys', 'distances_non', 'distances_ad', 'errors_rotation', 'errors_translation',...
