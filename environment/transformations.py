...
"""
Implements the disentangled transformations (ReAgent). Additionally, provides rotation conversions from pytorch3d.
"""
...
def to_disentangled(poses, pcd):
    ...
    poses[..., :3, 3] = poses[..., :3, 3] - mu \
                      + (poses[..., :3, :3] @ mu[..., None]).view(poses[..., :3, 3].shape)
    return poses
...
def nearest_neighbor(key, query, k=1):
    sqr_dist = square_distance(key, query)
    if k == 1:
        dist, idx = torch.where(sqr_dist > 1e-7, torch.sqrt(sqr_dist), torch.zeros_like(sqr_dist)).min(dim=1)
    else:
        dist, idx = torch.where(sqr_dist > 1e-7, torch.sqrt(sqr_dist), torch.zeros_like(sqr_dist)).topk(k, dim=1,
                                                                                                        largest=False)
    return dist, idx
